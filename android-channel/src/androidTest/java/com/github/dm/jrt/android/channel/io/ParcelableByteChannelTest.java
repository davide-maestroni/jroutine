...
public class ParcelableByteChannelTest extends ActivityInstrumentationTestCase2<TestActivity> {
  ...
  public void testAvailable() throws IOException {
    ...
    final Channel<ParcelableByteChunk, ParcelableByteChunk> channel =
        JRoutineService.on(ServiceSource.serviceOf(getActivity()))
                       .with(factoryOf(PassingInvocation.class))
                       .invoke();
    final ByteChunkOutputStream stream = ParcelableByteChannel.withOutput(channel)
  ...
  public void testBufferEquals() throws IOException {
    ...
    final Channel<?, ParcelableByteChunk> result = JRoutineService.on(ServiceSource.serviceOf(getActivity()))
                                                                  .with(factoryOf(
                                                                      PassingInvocation.class))
                                                                  .invoke()
                                                                  .pass(channel)
                                                                  .close();
    final ByteChunkOutputStream stream = ParcelableByteChannel.withOutput(channel).buildOutputStream();
  ...
  public void testInputClose() throws IOException {
    ...
    final ByteChunkInputStream inputStream = ParcelableByteChannel.getInputStream(buffer);
    inputStream.close();
    final byte[] b = new byte[16];
  ...
  public void testReadAll() throws IOException {
    ...
    stream.write(new byte[]{31, 17, (byte) 155, 13});
    stream.flush();
    final ByteChunkInputStream inputStream = ParcelableByteChannel.getInputStream(channel.next());
    final ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
  ...
  public void testReadByte() throws IOException {
    ...
    stream.flush();
    channel.close();
    inputStream = ParcelableByteChannel.getInputStream(result.in(seconds(10)).next());
    assertThat(inputStream.read()).isEqualTo((byte) 155);
    assertThat(inputStream.read()).isEqualTo(13);
    assertThat(inputStream.read()).isEqualTo(-1);
  }
