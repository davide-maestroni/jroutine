 ...
 */
class DefaultSparseChannelHandler implements SparseChannelHandler {
  ...
  private final Configurable<SparseChannelHandler> mConfigurable =
      new Configurable<SparseChannelHandler>() {

        @NotNull
        @Override
        public SparseChannelHandler withConfiguration(
            @NotNull final ChannelConfiguration configuration) {
          return DefaultSparseChannelHandler.this.withConfiguration(configuration);
        }
      };
   ...
   */
  DefaultSparseChannelHandler(@NotNull final ScheduledExecutor executor) {
  ...
  @NotNull
  @Override
  public <DATA, IN extends DATA> SparseArray<Channel<IN, ?>> inputOfParcelableFlow(
  ...
  public <IN> Channel<FlowData<? extends IN>, ?> mergeParcelableInputOf(
    ...
    final SparseArray<Channel<IN, ?>> channelArray =
        new SparseArray<Channel<IN, ?>>(channels.size());
  ...
  @NotNull
  @Override
  public <OUT> SparseArray<Channel<?, OUT>> outputOfParcelableFlow(
  ...
  @NotNull
  @Override
  public <OUT> SparseArray<Channel<?, OUT>> outputOfParcelableFlow(final int startId,
      final int rangeSize,
      @NotNull final Channel<?, ? extends ParcelableFlowData<? extends OUT>> channel) {
    ConstantConditions.positive("range size", rangeSize);
    final HashSet<Integer> idSet = new HashSet<Integer>();
  ...
  @NotNull
  @Override
  public Builder<? extends SparseChannelHandler> withChannel() {
    return new Builder<SparseChannelHandler>(mConfigurable, mConfiguration);
  }

  @NotNull
  @Override
  public SparseChannelHandler withConfiguration(@NotNull final ChannelConfiguration configuration) {
  ...
  @NotNull
  private <DATA, IN extends DATA> SparseArray<Channel<IN, ?>> internalInputOfFlow(
      @NotNull final Channel<? super ParcelableFlowData<DATA>, ?> channel,
      @NotNull final HashSet<Integer> ids) {
    final SparseArray<Channel<IN, ?>> channelArray = new SparseArray<Channel<IN, ?>>(ids.size());
    for (final Integer id : ids) {
  ...
  private <OUT> SparseArray<Channel<?, OUT>> internalOutputOfFlow(
      @NotNull final Channel<?, ? extends ParcelableFlowData<? extends OUT>> channel,
      @NotNull final HashSet<Integer> ids) {
    synchronized (sOutputChannels) {
      ...
      HashMap<FlowInfo, SparseArray<Channel<?, ?>>> channelArrays = outputChannels.get(channel);
      if (channelArrays == null) {
        channelArrays = new HashMap<FlowInfo, SparseArray<Channel<?, ?>>>();
      ...
      final SparseArray<Channel<?, OUT>> channelArray = new SparseArray<Channel<?, OUT>>(size);
      SparseArray<Channel<?, ?>> channels = channelArrays.get(flowInfo);
      if (channels != null) {
        final int channelSize = channels.size();
        for (int i = 0; i < channelSize; ++i) {
        ...
        final SparseArray<Channel<OUT, ?>> inputArray = new SparseArray<Channel<OUT, ?>>(size);
