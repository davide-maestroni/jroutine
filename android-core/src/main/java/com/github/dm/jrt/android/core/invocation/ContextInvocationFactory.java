...
public abstract class ContextInvocationFactory<IN, OUT> extends DeepEqualObject {
  ...
  /**
   * Implementation of an invocation factory adapting Context invocations to be used as common ones.
   *
   * @param <IN>  the input data type.
   * @param <OUT> the output data type.
   */
    ...
    private DefaultContextInvocationFactory(
      ...
      if (!Reflection.hasStaticScope(invocationClass)) {
        throw new IllegalArgumentException(
            "the invocation class must have a static scope: " + invocationClass.getName());
      }

      mFactory = InvocationFactory.factoryOf(invocationClass, args);
    }
  ...
  private static class WrappingContextInvocationFactory<IN, OUT>
    ...
    private WrappingContextInvocationFactory(@NotNull final InvocationFactory<IN, OUT> factory,
      ...
      if (!Reflection.hasStaticScope(factoryClass)) {
        throw new IllegalArgumentException(
            "the invocation factory class must have a static scope: " + factoryClass.getName());
      }
