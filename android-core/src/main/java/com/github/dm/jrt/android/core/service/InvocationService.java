...
public class InvocationService extends Service {
   ...
   */
  public static void putInvocation(@NotNull final Bundle bundle, @NotNull final String invocationId,
  ...
  private RoutineInvocation getInvocation(@NotNull final Message message) {
    ...
    synchronized (mMutex) {
      ...
      if (invocation == null) {
        ...
        throw new IllegalArgumentException(
            "[" + getClass().getName() + "] the Service message has invalid invocation ID: "
                + invocationId);
      }
  ...
  private void initRoutine(@NotNull final Message message) throws Exception {
    ...
    @SuppressWarnings("unchecked") final Class<? extends ContextInvocation<?, ?>> targetClass =
        (Class<? extends ContextInvocation<?, ?>>) data.getSerializable(KEY_TARGET_INVOCATION);
    if (targetClass == null) {
      mLogger.err("the Service message has no target invocation class");
      throw new IllegalArgumentException(
          "[" + getClass().getName() + "] the Service message has no target invocation class");
    }
      ...
      if (routineState == null) {
        ...
        if (logClass != null) {
          ...
          final Object[] logArgs =
              ((logValue != null) && (logValue.getValue() != null)) ? (Object[]) logValue.getValue()
                  : Reflection.NO_ARGS;
          try {
          ...
          if (data != null) {
            ...
            if (invocationId != null) {
              synchronized (service.mMutex) {
                final RoutineInvocation invocation = service.mInvocations.get(invocationId);
                if (invocation != null) {
                  invocation.recycle();
                }
              }
