...
public class LoaderStreamBuilderTest extends ActivityInstrumentationTestCase2<TestActivity> {
                             ...
                             .let(
                                 new Function<StreamBuilder<String, String>,
                                   ...
                                   public LoaderStreamBuilderCompat<String, String> apply(
                                       final StreamBuilder<String, String> builder) {
                                     return ((LoaderStreamBuilderCompat<String, String>) builder)
                                         .map(
                                         append("test2"));
                                   }
                                 })
        ...
        .<String>withStream().on(loaderFrom(activity))
                             .letWithConfig(
                                 new BiFunction<LoaderStreamConfigurationCompat,
                                     StreamBuilder<String, String>, StreamBuilder<String,
                                     String>>() {
  ...
  private static void testMapAllConsumer(@NotNull final FragmentActivity activity) {
    assertThat(JRoutineLoaderStreamCompat //
        ...
        }).call("test1", "test2", "test3").after(seconds(10)).all()).containsExactly(
        "test1test2test3");
    assertThat(JRoutineLoaderStreamCompat //
  ...
  private static void testMapFunction(@NotNull final FragmentActivity activity) {
    assertThat(JRoutineLoaderStreamCompat //
        ...
        }).call("test1", "test2").after(seconds(10)).all()).containsExactly("TEST1", "TEST2");
    assertThat(JRoutineLoaderStreamCompat //
        .<String>withStream().on(loaderFrom(activity))
                             .sorted()
                             .asyncParallel()
  ...
  private static BiFunction<LoaderStreamConfigurationCompat, Function<Channel<?, String>,
      Channel<?, String>>, Function<Channel<?, String>, Channel<?, String>>> transformBiFunction() {
    return new BiFunction<LoaderStreamConfigurationCompat, Function<Channel<?, String>,
      ...
      public Function<Channel<?, String>, Channel<?, String>> apply(
        ...
        assertThat(configuration.getLoaderContext()).isInstanceOf(LoaderContextCompat.class);
  ...
  public void testFactory() {
    final LoaderStreamBuilderCompat<String, String> builder =
        JRoutineLoaderStreamCompat.<String>withStream().on(loaderFrom(getActivity()))
                                                       .map(new UpperCase());
    assertThat(
        JRoutineCore.with(builder.buildFactory()).call("test").after(seconds(10)).next()).isEqualTo(
        "TEST");
    assertThat(JRoutineLoaderCompat.on(loaderFrom(getActivity()))
  ...
  public void testFlatMapNullPointerError() {
    try {
      JRoutineLoaderStreamCompat.withStream().on(loaderFrom(getActivity())).sync().flatMap(null);
      fail();
    ...
    try {
      JRoutineLoaderStreamCompat.withStream().on(loaderFrom(getActivity())).async().flatMap(null);
      fail();
  ...
  public void testMapAllFunctionNullPointerError() {
    try {
      JRoutineLoaderStreamCompat.withStream().on(loaderFrom(getActivity())).async().mapAll(null);
      fail();
  ...
  public void testMapConsumerNullPointerError() {
    try {
      JRoutineLoaderStreamCompat.withStream().on(loaderFrom(getActivity())).async().mapAccept(null);
      fail();
  ...
  public void testStreamGet() {
    assertThat(JRoutineLoaderStreamCompat.withStreamGet(constant("test")).immediate().close().all())
  ...
  public void testStreamGetAbort() {
    Channel<String, String> channel =
        JRoutineLoaderStreamCompat.withStreamGet(constant("test")).immediate().immediate().call();
    assertThat(channel.abort()).isTrue();
    assertThat(channel.getError()).isInstanceOf(AbortException.class);
  ...
  public void testStreamOf() {
    assertThat(JRoutineLoaderStreamCompat.withStreamOf("test")
                                         ...
                                         .after(seconds(10))
                                         .all()).containsExactly("test");
    assertThat(JRoutineLoaderStreamCompat.withStreamOf("test1", "test2", "test3")
                                         .on(loaderFrom(getActivity()))
                                         .close()
                                         .after(seconds(10))
                                         .all()).containsExactly("test1", "test2", "test3");
    assertThat(JRoutineLoaderStreamCompat.withStreamOf(Arrays.asList("test1", "test2", "test3"))
                                         .on(loaderFrom(getActivity()))
                                         .close()
                                         .after(seconds(10))
                                         .all()).containsExactly("test1", "test2", "test3");
  ...
  public void testStreamOfAbort() {
    Channel<String, String> channel =
        JRoutineLoaderStreamCompat.withStreamOf("test").on(loaderFrom(getActivity())).call();
    assertThat(channel.abort()).isTrue();
    assertThat(channel.after(seconds(10)).getError()).isInstanceOf(AbortException.class);
    channel = JRoutineLoaderStreamCompat.withStreamOf("test1", "test2", "test3")
                                        .on(loaderFrom(getActivity()))
                                        .call();
    assertThat(channel.abort()).isTrue();
    assertThat(channel.after(seconds(10)).getError()).isInstanceOf(AbortException.class);
    channel = JRoutineLoaderStreamCompat.withStreamOf(Arrays.asList("test1", "test2", "test3"))
                                        .on(loaderFrom(getActivity()))
                                        .call();
    assertThat(channel.abort()).isTrue();
    assertThat(channel.after(seconds(10)).getError()).isInstanceOf(AbortException.class);
  ...
  private static class RetryFunction implements Function<Object, Channel<Object, String>> {
    ...
    private static Channel<Object, String> apply(final Object o,
        @NotNull final FragmentActivity activity, @NotNull final Routine<Object, String> routine,
