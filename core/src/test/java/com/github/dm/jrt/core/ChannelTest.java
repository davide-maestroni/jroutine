...
public class ChannelTest {
  ...
  public void testNextList() {
    ...
    try {
      JRoutineCore.io().buildChannel().pass("test1").eventuallyAbort().after(seconds(1)).next(2);
      fail();
    ...
    try {
      JRoutineCore.io().buildChannel().pass("test1").eventuallyFail().after(seconds(1)).next(2);
      fail();
  ...
  public void testNextOr() {
    assertThat(
        JRoutineCore.io().buildChannel().pass("test1").after(seconds(1)).nextOrElse(2)).isEqualTo(
        "test1");
    assertThat(JRoutineCore.io()
                           .buildChannel()
                           .eventuallyContinue()
    ...
    try {
      JRoutineCore.io().buildChannel().eventuallyAbort().after(millis(100)).nextOrElse("test2");
      fail();
    ...
    try {
      JRoutineCore.io().buildChannel().eventuallyFail().after(millis(100)).nextOrElse("test2");
      fail();
  ...
  public void testOf() {
    ...
    assertThat(
        JRoutineCore.io().of(Arrays.asList(3, 12, -7)).after(seconds(1)).all()).containsExactly(3,
        12, -7);
  }
  ...
  public void testOrderType() {
    ...
    channel1.after(millis(200)).pass(23).now().pass(-77L).close();
    ...
    channel2.after(millis(200)).pass(23).now().pass(-77L).close();
  ...
  public void testPartialOut() {
    ...
    assertThat(outputChannel.now().getComplete()).isFalse();
    channel.close();
    assertThat(channel.isOpen()).isFalse();
