...
/**
 * A gate represents {@link Spring} with the ability to recharge the same
 * kind of data after a given delay.<br/>
 * Its methods can be called only inside a {@link com.bmd.wtf.dam.Dam} and only from the same
 * thread where the <code>Dam</code> methods are invoked. Any different usage will cause an
