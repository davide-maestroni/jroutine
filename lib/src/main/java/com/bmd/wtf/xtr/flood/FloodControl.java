 ...
 * by inserting into it {@link com.bmd.wtf.xtr.flood.Levee}s controlled by one or more
 * {@link FloodObserver}s.
 * <p/>
...
public class FloodControl<IN, OUT, CONTROLLER extends FloodObserver<IN, OUT>> {
    ...
    private final Class<CONTROLLER> mFacade;
    ...
    /**
     * Creates a flood control handled by controllers presenting the specified facade.
     * <p/>
     * Note that only interfaces can be passed as facade.
     *
     * @param facade The
     */
    public FloodControl(final Class<CONTROLLER> facade) {

        if (!facade.isInterface()) {

            throw new IllegalArgumentException(facade.getSimpleName() + " is not an interface");
        }
    ...
    public <OBSERVER extends CONTROLLER> CONTROLLER controlling(final OBSERVER observer) {
        ...
        if (observer == null) {
            ...
            throw new IllegalArgumentException("Observer instance cannot be null");
        }
    ...
    public <OBSERVER extends CONTROLLER> Levee<IN, OUT, CONTROLLER> leveeControlledBy(
        ...
        final Class<CONTROLLER> facade = mFacade;

        final Object mutex = new Object();

        //noinspection unchecked
        final CONTROLLER controller = (CONTROLLER) Proxy
                .newProxyInstance(facade.getClassLoader(), new Class<?>[]{facade},
