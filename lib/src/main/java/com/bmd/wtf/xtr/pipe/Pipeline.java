 ...
 * <p/>
 * Flowing data are filtered in a thread safe way by the use of
 * {@link Duct}s.
 * <p/>
...
public class Pipeline<SOURCE, IN, OUT> {
    ...
    public <NOUT> Stream<SOURCE, NOUT, NOUT> thenFlowingThrough(final Duct<OUT, NOUT> duct) {
        ...
        return mInputStream.thenFlowingThrough(new Dam<OUT, NOUT>() {

            @Override
            public Object onDischarge(final Floodgate<OUT, NOUT> gate, final OUT drop) {

                return duct.onDischarge(spring, drop);
            }

            @Override
            public Object onPullDebris(final Floodgate<OUT, NOUT> gate, final Object debris) {

                return duct.onPullDebris(spring, debris);
            }

            @Override
            public Object onPushDebris(final Floodgate<OUT, NOUT> gate, final Object debris) {

                return duct.onPushDebris(spring, debris);
            }

            @Override
            public Object onFlush(final Floodgate<OUT, NOUT> gate) {

                return duct.onFlush(spring);
            }
