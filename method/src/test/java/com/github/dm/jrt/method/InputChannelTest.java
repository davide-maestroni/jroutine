...
public class InputChannelTest {
  ...
  public void testAbort() {
    ...
    final InputChannel<String> channel = RoutineMethod.inputChannel();
  ...
  public void testMaxSize() {
    try {
      RoutineMethod.toInput(
          JRoutineCore.io().applyChannelConfiguration().withMaxSize(1).configured().buildChannel())
                   .pass("test1", "test2");
      fail();
  ...
  @Test
  public void testOf() {
    final InputChannel<Integer> channel = RoutineMethod.inputOf(2);
    assertThat(channel.isOpen()).isFalse();
    assertThat(channel.after(seconds(1)).all()).containsExactly(2);
    assertThat(RoutineMethod.inputOf().after(seconds(1)).all()).isEmpty();
    assertThat(RoutineMethod.inputOf(-11, 73).after(seconds(1)).all()).containsExactly(-11, 73);
    assertThat(
        RoutineMethod.inputOf(Arrays.asList(3, 12, -7)).after(seconds(1)).all()).containsExactly(3,
        12, -7);
  }
  ...
  public void testPendingInputs() throws InterruptedException {
    final InputChannel<Object> channel = RoutineMethod.inputChannel();
    assertThat(channel.isOpen()).isTrue();
    channel.pass("test");
    assertThat(channel.isOpen()).isTrue();
    channel.after(millis(500)).pass("test");
    assertThat(channel.isOpen()).isTrue();
    final InputChannel<Object> outputChannel = RoutineMethod.inputChannel();
