...
public class SequencesTest {
  ...
  public void testRange() {
    assertThat(
        ...
        }))).close().inMax(seconds(3)).all()).containsExactly('a', 'b', 'c', 'd', 'e');
    ...
    assertThat(JRoutineCore.with(consumerCommand(range(0, 2, 0.7))).close().inMax(seconds(3)).all())
    ...
    assertThat(
        JRoutineCore.with(consumerCommand(range(0, -5))).close().inMax(seconds(3)).all()).isEqualTo(
